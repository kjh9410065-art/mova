...
export default function RecommendPage() {
  ...
  const intent = useMemo(() => parseRecommendationIntent(query), [query]);
  ...
  const results = useMemo(() => {
    ...
    if (!query.trim()) return ranked.slice(0, 8).map((item, index) => ({ ...item.service, score: Math.max(0, Math.min(100, Math.round(item.score))), rank: index + 1, reason: item.service.bestFor, searchScore: 0 }));
    const searched = rankSearchResults(ranked.map((item) => item.service), query);
    const searchMap = new Map(searched.map((service) => [service.id, scoreSearch(service, query)]));
  ...
  // 검색 버튼은 현재 입력값을 기준으로 추천 결과를 다시 계산하고 첫 결과부터 보여줍니다.
  const submitSearch = (event) => { event.preventDefault(); setResultLimit(5); };
  // 빠른 조건 버튼은 검색어를 비우고 선택한 조건으로 추천 결과를 즉시 갱신합니다.
  const applyQuickCondition = (type) => { setQuery(""); setResultLimit(5); if (type === "free") setBudget("free"); if (type === "video") { setGoal("video"); setFeature("영상"); } if (type === "image") { setGoal("image"); setFeature("이미지"); } };
  const resetConditions = () => { setQuery(""); setGoal("shorts"); setBudget("low"); setSkill("easy"); setFeature("all"); setResultLimit(5); };
  ...
  return <main className="recommendPage"><div className="recommendShell">
    ...
    <section className="recommendSearch recommendIntentSearch"><label htmlFor="recommend-query">원하는 조건을 한 문장으로 입력해도 됩니다.</label><form className="recommendSearchRow" onSubmit={submitSearch}><input id="recommend-query" value={query} onChange={(event) => { setQuery(event.target.value); setResultLimit(5); }} placeholder="예: 무료로 쇼츠 만들고 싶어" autoComplete="off"/><button type="submit" className="recommendSearchButton">검색</button><button type="button" className="recommendResetButton" onClick={resetConditions}>조건 초기화</button></form>{intent.labels.length > 0 && <div className="intentLabels" aria-label="검색어에서 인식한 조건">{intent.labels.map((label) => <span key={label}>{label}</span>)}</div>}</section>
    ...
    <section className="conditionCard featureCondition"><div className="conditionTitleRow"><h2>04. 가장 중요한 기능</h2></div><div className="choiceGrid">{featureOptions.map(([id, label]) => <button type="button" className={`choice ${effectiveFeature === id ? "active" : ""}`} key={id} onClick={() => { setFeature(id); setQuery(""); setResultLimit(5); }}>{label}</button>)}</div></section>
    <section className="recommendResult"><div className="resultHead"><div><div className="eyebrow">STEP 02 · RESULT</div><h2>{query ? "입력한 조건에 맞는 서비스" : "당신에게 맞는 서비스"}</h2></div><div className="resultHeadActions"><a className="catalogLink" href="/catalog">전체 {catalog.length}개 보기</a></div></div>
